...
// The delay is OFF for 8 beats, then ON full at beat 8, then OFF again at beat 12.
// Very obvious: you hear a dry riff, then it suddenly gets drenched in delay, then dry again.
pattern riff
  instrument jpluck
  octave 4
...
`))r.startsWith("# ")?a=r.slice(2).trim():a&&r.trim()&&(e.set(a,r.trim()),a="");return e}const Pc=Sc(xc);class Ac{#e=j("");get current(){return t(this.#e)}set current(e){p(this.#e,e,!0)}set(e){this.current=Pc.get(e)??""}clear(){this.current=""}}const ue=new Ac;var Tc=T("<button> </button>"),Dc=T('<span class="hover-help svelte-1h259us"> </span>'),Rc=T('<button class="transport-btn svelte-1h259us">⏹</button>'),qc=T('<button class="transport-btn svelte-1h259us">⏹</button>'),Cc=T('<button class="focus-clear svelte-1h259us" title="Clear focus">&times;</button>'),Lc=T('<div><span class="focus-label svelte-1h259us">Focus</span> <input class="focus-input svelte-1h259us" type="number" step="0.1" min="0"/> <span class="focus-sep svelte-1h259us">&ndash;</span> <input class="focus-input svelte-1h259us" type="number" step="0.1" min="0"/> <!></div>'),Mc=T('<span class="error svelte-1h259us"> </span>'),Ec=T('<div class="transport svelte-1h259us"><button class="transport-btn svelte-1h259us"> </button> <!> <!> <div class="volume-control svelte-1h259us"><span class="volume-icon svelte-1h259us">&#9834;</span> <input class="volume-slider svelte-1h259us" type="range" min="0" max="1" step="0.01" aria-label="Volume"/></div> <!> <!></div>'),Fc=T('<div class="top-bar svelte-1h259us"><div class="mode-tabs svelte-1h259us"><span class="brand svelte-1h259us">Get</span> <!></div> <!> <!> <button class="theme-toggle svelte-1h259us" title="Toggle theme"> </button></div>');function Oc(n,e){ye(e,!0);let a=j("stopped"),r=j("not-initialized"),s=j(null),i=null,o=j(null),c=null,l=j(.75);He(()=>{e.bridge.onObserve(M=>{Pe.updatePosition(M.position)})});function f(){u(),c=setInterval(()=>e.bridge.observe(),50)}function u(){c&&(clearInterval(c),c=null)}function d(M){M.target instanceof HTMLInputElement||M.target instanceof HTMLTextAreaElement||M.target instanceof HTMLSelectElement||(M.code==="Space"?(M.preventDefault(),_()):M.code==="Escape"&&(t(a)!=="stopped"||Cn.playing)&&(M.preventDefault(),h()))}He(()=>(window.addEventListener("keydown",d),()=>window.removeEventListener("keydown",d))),He(()=>{e.mode==="instrument"&&!Cn.playing&&t(a)==="playing"&&(p(a,"stopped"),Pe.playing=!1,Pe.updatePosition(null),u())}),He(()=>{Fn.mutedPartTracks,Fn.soloPartTracks,Fn.mutedPatternLabels,Fn.soloPatternLabels;const M=Pe.sourceMap,V=Pe.position,U=ee.songAst;if(!Pe.playing){e.bridge.setMuteMask(0);return}const G=V?.part_idx??ee.selectedPartIndex,Y=U?.score.sequence[G]?.part_name??null,re=Fn.computeMask(M,G,U,Y);e.bridge.setMuteMask(re)}),He(()=>{const M=ee.text,V=ee.selectedSection;if(!(t(a)!=="playing"||!V)&&e.mode!=="instrument")return i&&clearTimeout(i),i=setTimeout(async()=>{try{const U=M.endsWith(`
`)?M:M+`
...
`,n=n.substring(m.length+1),l=!0),!l){let w=this.rules.other.nextBulletRegex(_),h=this.rules.other.hrRegex(_),A=this.rules.other.fencesBeginRegex(_),C=this.rules.other.headingBeginRegex(_),F=this.rules.other.htmlBeginRegex(_),P=this.rules.other.blockquoteBeginRegex(_);for(;n;){let y=n.split(`
`,1)[0],g;if(m=y,this.options.pedantic?(m=m.replace(this.rules.other.listReplaceNesting,"  "),g=m):g=m.replace(this.rules.other.tabCharGlobal,"    "),A.test(m)||C.test(m)||F.test(m)||P.test(m)||w.test(m)||h.test(m))break;if(g.search(this.rules.other.nonSpaceChar)>=_||!m.trim())u+=`
